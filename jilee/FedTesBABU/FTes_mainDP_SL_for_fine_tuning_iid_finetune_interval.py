...
def main():
    ...
    makedir(model_dir)
    log_directory = f"checkpoints/{str(args.iid)}/{start_time}/'tuning with interval without augmented dataset'/"
    ...
    for epoch in range(num_train_epochs):
        ...
        #Gm_manifold.append(torch.unsqueeze(net_glob.prototype_vectors.data.to(device), 0).to(device))
        ...
        #global_test_results2 = local_test_global_model_proto(args, net_glob, X_server, y_server, coefs)   
        
        log('Global model test')
        log(global_test_results)
        #log(global_test_results2)
