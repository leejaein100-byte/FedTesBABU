...
class TESNet(nn.Module):
    def __init__(self, features, img_size, prototype_shape,
        ...
        # CHANGE 1: Define prototype_per_class explicitly for BayesianKDTrainer compatibility
        self.prototype_per_class = self.num_prototypes // self.num_classes
        self.num_prototypes_per_class = self.prototype_per_class # Keep for backward compatibility
    ...
    def _cosine_convolution(self, x):
        ...
        distances = -distances # This makes it "min distance" (negative similarity)
        return distances
    ...
    def forward(self, x):
        ...
        # cosine_distances is "negative similarity" (closer = smaller/more negative)
        ...
        # CHANGE 2: Return 3 values to match train_and_test.txt structure
        # (logits, min_distance_metric, max_similarity_metric)
        return logits, cosine_min_distances, project_max_distances
...
def construct_TesNet(args, base_architecture, pretrained=True, img_size=224,
                    prototype_shape=(2000, 128, 1, 1), num_classes=200,
                    prototype_activation_function='log',
                    prototype_per_class=10, dataset='cub',
                    add_on_layers_type='bottleneck'):
