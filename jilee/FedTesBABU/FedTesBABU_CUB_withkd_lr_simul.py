...
def enhanced_training_with_bayesian_kd_server(args, clients, clients_state_list, net_glob, dict_users, server_idx, dataset, 
    ...
    kd_trainer = BayesianKDTrainer(args, num_classes, net_glob, prototype_per_class, alpha=1.0, temp=args.temp, hyperparam = args.hyperparam)
...
def main():
    ...
    for trial in range(3):
        ...
        model_dir = './saved_models/' + dataset_name+'/' + base_architecture + '/' + str(args.iid) + '/'+str(start_time) + '/' + 'FedTesBABU_with_KD_Noncayley_lr_simul'
        if os.path.exists(model_dir) is True:
            shutil.rmtree(model_dir)
        makedir(model_dir)
        log_directory = f"checkpoints/{str(args.iid)}/{str(dataset_name)}/'FedTesBABU_with_KD_NonCayley_lr_simul'/{start_time+'without augmented dataset'}/"
