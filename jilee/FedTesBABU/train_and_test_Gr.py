...
def get_input_logits(inputs, model, is_logit = False, device=None):
...
class BayesianKDTrainer:
    ...
    def get_ensemble_logits(
        ...
        for i, teacher in enumerate(teachers):
            ...
            logit_or_prob = get_input_logits(inputs, t, is_logit=False, device=device)  # (B, C)
    ...
    def train_global_model_with_kd(self, teacher_models, server_data, device):
        ...
        for batch_inputs, batch_targets in server_loader:
            ...
            total_loss = self.knowledge_distillation_loss(project_max_distances, teacher_logits)
            _, predicted1 = torch.max(project_max_distances, 1)
    ...
    def knowledge_distillation_loss(self, student_score, teacher_logits, temperature=None):
        """Compute knowledge distillation loss"""
...
def _train_or_test(args, client_idx, client_model, X_data, y_data, body_train, is_train, class_specific=True, use_l1_mask=True,
    ...
    if body_train:
        ...
        #find_parameter_source(client_model, optimizer_specs)
        #check_parameter_collection_bug(client_model)
    ...
    for epoch in range(train_epochs):
        ...
        for i, (image, label) in enumerate(dataloader):
            ...
            with grad_req:
                output, cosine_min_distances, project_max_distances = client_model(image)    #cosine min distances는 score 점수,즉 무조건 양수
                del image
                ...
                if class_specific:
                    ...
                    masked = (-cosine_min_distances).masked_fill(prototypes_of_wrong_class == 0, float('-inf'))
            ...
            if is_train:
                if not class_specific:
                    ...
                    #coefs['clst'] * cluster_cost +
                        ...
                        loss = cross_entropy  - 0.08 * separation_cost - 1e-7 * subspace_sep + 1e-4 * l1
                        # + 0.8 * cluster_cost
    ...
    with torch.no_grad():
        for i, (image, label) in enumerate(test_loader):
            ...
            inverted_distances_to_nontarget_prototypes, _ = \
                torch.max((cosine_min_distances) * prototypes_of_wrong_class, dim=1)
            separation_cost = torch.mean(- inverted_distances_to_nontarget_prototypes)
            ...
            projection_operator_2 = torch.unsqueeze(subspace_basis_matrix_T, dim = 1)
            #[200,10,128]->[200,128,10]
        
            del subspace_basis_matrix_T

            projection_operator = torch.matmul(projection_operator_1,projection_operator_2)#[200,128,10] [200,10,128] -> [200,128,128]
            pairwise_distance= torch.norm(projection_operator, dim = [2,3])
            subspace_sep = 0.5 * torch.norm(client_model.prototype_per_class-pairwise_distance, p=1, dim = [0,1], dtype=torch.double) / torch.sqrt(torch.tensor(2,dtype=torch.double)).cuda()
            del projection_operator_1
            del projection_operator_2
            del pairwise_distance
