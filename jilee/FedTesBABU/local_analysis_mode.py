...
def _build_dataset_index_metadata(data_args, indices, sample_idx):
    raw_dataset = StanfordDogsDataset(data_args, transform=None)
    if sample_idx < 0 or sample_idx >= len(indices):
        raise IndexError(f"sample_idx={sample_idx} out of range (0..{len(indices)-1})")

    dataset_idx = int(indices[sample_idx])
    sample_path = raw_dataset.image_paths[dataset_idx]
    sample_label = int(raw_dataset.labels[dataset_idx])
    sample_class_name = raw_dataset.class_names[sample_label]
    return {
...
def _get_client_test_metadata(data_args, client_idx, sample_idx):
    dict_users, server_idx, dataset = setup_datasets(data_args)
    y_all = dataset["mixed"]["y"]

    if dataset.get("iid_mode", True):
        if client_idx not in dict_users:
            raise RuntimeError(f"Client {client_idx} not found in dict_users.")
        user_indices = dict_users[client_idx]
        user_labels = [y_all[idx].item() for idx in user_indices]
        _, test_indices = train_test_split(
            user_indices,
            train_size=getattr(data_args, "tr_frac", 0.8),
            stratify=user_labels,
            random_state=data_args.seed,
        )
    else:
        _, dict_users_test = dict_users
        if client_idx not in dict_users_test or not dict_users_test[client_idx]:
            raise RuntimeError(f"Client {client_idx} has empty test split.")
        test_indices = dict_users_test[client_idx]

    return _build_dataset_index_metadata(data_args, test_indices, sample_idx)


def _get_global_test_metadata(data_args, sample_idx):
    dict_users, server_idx, dataset = setup_datasets(data_args)
    y_all = dataset["mixed"]["y"]
    global_test_indices = []
    ...
    for client_idx in range(data_args.num_users):
        if dataset.get("iid_mode", True):
            ...
            _, dict_users_test = dict_users
            test_indices = dict_users_test.get(client_idx, [])
        global_test_indices.extend(test_indices)

    if not global_test_indices:
        raise RuntimeError("Global test split is empty.")

    return _build_dataset_index_metadata(data_args, global_test_indices, sample_idx)
...
def _get_class_names(data_args):
    if data_args.dataset == "Stanford_dog":
        ...
        images_dir = "/data2/data/cropped_Tesla/integrated" if data_args.use_bbox else "/data2/data/StanfordCars/integrated"
    else:
        return None
...
def main():
    ...
    if args.image_path is not None:
        ...
        sample_metadata = {
            "sample_path": args.image_path,
            "sample_class_name": _label_to_class_name(data_args, label),
        ...
        sample_metadata = _get_global_test_metadata(data_args, args.sample_idx)
        ...
        sample_metadata = _get_client_test_metadata(data_args, args.client_idx, args.sample_idx)
    ...
    gt_label_one_based = label + 1 if label >= 0 else None

    log(f"Predicted class: {pred_cls}")
    log(f"Ground truth label (0-based): {label}")
    if gt_label_one_based is not None:
        log(f"Ground truth label (1-based): {gt_label_one_based}")
    if gt_label_name is not None:
        log(f"Ground truth label name: {gt_label_name}")
    if sample_metadata is not None:
        if sample_metadata.get("sample_label") is not None:
            log(f"Dataset sample label from path mapping: {sample_metadata['sample_label']}")
        if sample_metadata.get("sample_class_name") is not None:
            log(f"Dataset sample class folder: {sample_metadata['sample_class_name']}")
        if sample_metadata.get("dataset_idx") is not None:
            log(f"Dataset sample index: {sample_metadata['dataset_idx']}")
        if sample_metadata.get("sample_path") is not None:
            log(f"Dataset sample path: {sample_metadata['sample_path']}")
    ...
    summary = {
        ...
        "ground_truth_label_one_based": None if label < 0 else int(label + 1),
