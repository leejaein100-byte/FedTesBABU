...
def create_push_dataloader(args, dataset, server_idx, dict_users, device):
    ...
    for client_idx in range(args.num_users):
        X_train, y_train = X_train, y_train = load_data(args, dataset, server_idx, client_idx, dict_users, train = True, private = True)
...
def main():
    ...
    makedir(model_dir)
    log_directory = f"checkpoints/{str(args.iid)}/'FedTes_Stiefel_CUB'/{start_time+'without augmented dataset'}/"
    ...
    for epoch in range(num_train_epochs):
        ...
        for client_idx in range(args.num_users):
            ...
            X_client_test,y_client_test = load_data(args, dataset, server_id, client_idx, dict_users_train, train = True, private = False)
        ...
        # Stack client prototypes along a new dimension (dim 0)
        ...
        for client_idx in range(args.num_users):
            X_train, y_train = load_data(args, dataset, server_id, client_idx, dict_users_train, train = False, private = True)
            client_test_results = local_test_global_model(args, clients[client_idx], X_client_test, y_client_test, coefs)
