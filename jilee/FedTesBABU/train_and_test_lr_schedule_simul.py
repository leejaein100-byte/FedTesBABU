...
class BayesianKDTrainer:
    ...
    def get_ensemble_teacher_probs(self, teachers, inputs, device=None):
        """
        Build teacher targets as:
        1) softmax each teacher logits
        2) average probabilities across teachers
        3) temperature sharpen averaged probabilities
        """
        if device is None:
            device = inputs.device
        B = inputs.size(0)
        Tn = len(teachers)
        logits = torch.zeros(B, Tn, self.num_classes, device=device)

        for i, teacher in enumerate(teachers):
            t = teacher.module if hasattr(teacher, 'module') else teacher
            t.eval().to(device)
            logits[:, i, :] = get_input_logits(
                inputs, t, score_logit=self.args.score_logit, is_logit=True, device=device)

        teacher_probs = F.softmax(logits, dim=-1).mean(dim=1)  # (B, C)
        teacher_probs = temp_sharpen(teacher_probs, dim=-1, temp=self.temp)
    ...
    def knowledge_distillation_loss(self, student_score, teacher_targets):
        """Compute KD loss with teacher targets already in probability space."""
        T = self.temp
        log_p_s = F.log_softmax(student_score / T, dim=1)
        p_t = torch.clamp(teacher_targets, min=1e-8)  #여기가 바뀐 부분.
    ...
    def knowledge_distillation_loss_CE(self, student_score, teacher_targets):
        """Compute knowledge distillation loss using Cross Entropy"""
        T = self.temp
        
        # Teacher targets are already probabilities (softmax -> average -> sharpen)
        p_t = torch.clamp(teacher_targets, min=1e-8)
        p_t = p_t / p_t.sum(dim=1, keepdim=True)
