...
class TESNet_Grassmann(nn.Module):
    def __init__(self, features, img_size, prototype_shape, dataset, prototype_per_class, proto_layer_rf_info,
    ...
    def global_min_pooling(self, distances):   #기존: (N,M*C,H,W) 내 코드: (N,C,H,W)
        ...
        #min_distances = -F.max_pool2d(-distances,
                                      #kernel_size=(distances.size()[2],
                                                   #distances.size()[3]))
        min_distances, _ = torch.topk(-distances, self.prototype_per_class, dim=2)  #shape: (N,C,3)      
    ...
    def global_max_pooling(self, distances):    # projected_vectors
        ...
        max_distances, _ = torch.topk(distances, self.prototype_per_class, dim=2)
    ...
    def forward(self, x):
        ...
        return  logits, cosine_min_distances, project_max_distances    #, input_dim    #, sel_projected_vectors
...
def construct_TesNet(base_architecture, prototype_per_class, dataset = 'cifar10', pretrained=True, img_size=224, 
    ...
    return TESNet_Grassmann(features=features,
                 img_size=img_size,
