...
import settings_CUB
from utils.Fedtes_data_dist_img_augment_iid import *
...
def main():
    ...
    model_dir = './saved_models/' + dataset_name+'/' + base_architecture + '/' + str(start_time) + '/'

    if os.path.exists(model_dir) is True:
        shutil.rmtree(model_dir)
    makedir(model_dir)
    log_directory = f"checkpoints/{start_time+'augmented dataset IID'}/"
    ...
    # Use the IID augmented dataset distribution
    user_datasets = load_cropped_CUB_random_distribution(
        num_users=args.num_users, 
        server_id_size=args.server_id_size, 
        tr_frac=0.8, 
        seed=42, 
    ...
    # Note: We're now using the IID dataset directly, so we don't need the original setup_datasets
    ...
    # Prepare combined test data from all clients
    X_test = []
    y_test = []
    for client_idx in range(args.num_users):
        # Load test data for each client using the IID distribution
        _, _, X_temp_test, y_temp_test = load_data_random(user_datasets, client_idx, private=True)
    ...
    for epoch in range(num_train_epochs):
        ...
        for client_idx in range(args.num_users):
            ...
            # Load training data for this client using the IID distribution
            X_train, y_train, _, _ = load_data_random(user_datasets, client_idx, private=True)
            
            clients[client_idx], loss_dict = train(args, client_idx, clients[client_idx], X_train, y_train, 
                                                 prototype_per_class, is_train=True, body_train=True, 
                                                 coefs=coefs, log=print)
        ...
        # Properly handle the dimensions - avoid using squeeze which can remove too many dimensions
        Gm_manifold = torch.cat(Gm_manifold, dim=0)
        
        # Print shape for debugging
        print("Shape before permute:", Gm_manifold.shape)
        
        # For a 4D tensor, permute dimensions
        if Gm_manifold.dim() == 4:
            Gm_manifold = Gm_manifold.permute(1, 0, 2, 3)
        else:
            # Add proper dimension handling here based on the actual shape
            print(f"Warning: Gm_manifold has unexpected shape: {Gm_manifold.shape}")
            # Add appropriate reshaping logic here
        ...
        for client_idx in range(args.num_users):
            # Load test data for this client using the IID distribution
            _, _, X_client_test, y_client_test = load_data_random(user_datasets, client_idx, private=True)
    ...
    for client_idx in range(args.num_users):
        ...
        # Load training and test data for this client using the IID distribution
        X_train, y_train, X_client_test, y_client_test = load_data_random(user_datasets, client_idx, private=True)
        
        for fine_tune_epoch in range(args.fine_tune_epochs):
            clients[client_idx] = torch.nn.DataParallel(clients[client_idx])
            clients[client_idx], loss_dict = fine_tune_train(
                args, client_idx, clients[client_idx], X_train, y_train, 
                prototype_per_class, is_train=True, body_train=False, coefs=coefs, log=print
            )
            clients[client_idx] = clients[client_idx].module
            
            # Test this client after fine-tuning
    ...
    try:
        ...
        for client_idx in range(args.num_users):
            plt.plot(range(args.fine_tune_epochs), client_ft_results[client_idx], label=f'Client {client_idx}')
        
        plt.xlabel('Fine-tune Epoch')
        plt.ylabel('Test Accuracy')
        plt.title('Client Test Accuracies During Fine-tuning (IID Distribution)')
        plt.legend()
        plt.grid(True)
        
        # Save the figure
        plt.savefig(os.path.join(img_dir, 'client_accuracies_iid.png'))
        
        # Add to TensorBoard
        writer.add_figure('Client_Accuracies_IID', plt.gcf())
    except Exception as e:
        log(f"Error creating visualization: {str(e)}")
    
    log('Fine-tuning complete with IID augmented dataset distribution')
    writer.close()
    logclose()
